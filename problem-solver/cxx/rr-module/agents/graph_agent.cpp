...
void ScAgentCreateGraph::GetOrCreatePage(ScAddr & pageNode, std::string const & pageName)
{
...
void ScAgentCreateGraph::CreateMainIdntf(ScAddr const & node, std::string const & idtf)
{
...
ScResult ScAgentCreateGraph::DoProgram(ScActionInitiatedEvent const & event, ScAction & action)
{
  ...
  for (auto & page : pagesData)
  {
    ...
    ScAddr const & visitorsClassConnector =
        m_context.GenerateConnector(ScType::ConstPermPosArc, ScGraphKeynodes::concept_number, visitorsNode);
    ...
    ScAddr const & nrel_visitorsConnector =
        m_context.GenerateConnector(ScType::ConstPermPosArc, ScGraphKeynodes::nrel_visitors_count, visitorsConnector);
    ...
    ScAddr const & spentTimeClassConnector =
        m_context.GenerateConnector(ScType::ConstPermPosArc, ScGraphKeynodes::concept_number, spentTimeNode);
    ...
    ScAddr const & nrel_spent_timeConnector =
        m_context.GenerateConnector(ScType::ConstPermPosArc, ScGraphKeynodes::nrel_spent_time, spentTimeConnector);
